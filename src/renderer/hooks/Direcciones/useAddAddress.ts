...
export const useAddAddress = ({ isValid, address }: IDataRequestAddCliente): IDataResponse => {
  ...
  useEffect(() => {
    if (isValid && calls === 0) {
      ...
      const callContractFunction = async (): Promise<void> => {
        setIsLoading(true);
        try {
            const response = await window.electron.ipcRenderer.AddAddress(address);
            setIsSuccess(true);
            setResult(response);
        } catch (err) {

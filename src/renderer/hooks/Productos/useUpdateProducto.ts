...
export const useUpdateProducto = ({ isValid, producto }: IDataRequestAddCliente): IDataResponse => {
  ...
  useEffect(() => {
    if (isValid && calls === 0) {
      ...
      const callContractFunction = async (): Promise<void> => {
        setIsLoading(true);
        try {
            const response = await window.electron.ipcRenderer.UpdateProduct(producto);
            setIsSuccess(true);
            setResult(response);
        } catch (err) {

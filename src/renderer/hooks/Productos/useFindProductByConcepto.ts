...
import { IDataRequestSearchWithStringAndPagination, IProducto } from '../../../main/interfaces';
...
export const useFindProductByConcepto = ({ isValid, match, page, sizePage }: IDataRequestSearchWithStringAndPagination
): IDataResponse => {
  ...
  useEffect(() => {
    if (isValid && calls === 0) {
      ...
      const callContractFunction = async (): Promise<void> => {
        setIsLoading(true);
        try {
            const response = await window.electron.ipcRenderer.FindProducto(match, {page, sizePage});
            setIsSuccess(true);
            setResult(response);
        } catch (err) {

...
export const useAddProducto = ({ isValid, producto }: IDataRequestAddCliente): IDataResponse => {
  ...
  useEffect(() => {
    if (isValid && calls === 0) {
      ...
      const callContractFunction = async (): Promise<void> => {
        setIsLoading(true);
        try {
            const response = await window.electron.ipcRenderer.AddProduct(producto);
            setIsSuccess(true);
            setResult(response);
        } catch (err) {

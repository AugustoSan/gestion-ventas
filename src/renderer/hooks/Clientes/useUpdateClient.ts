...
export const useUpdateClient = ({ isValid, client }: IDataRequestUpdateCliente): IDataResponse => {
  ...
  useEffect(() => {
    if (isValid && calls === 0) {
      ...
      const callContractFunction = async (): Promise<void> => {
        setIsLoading(true);
        try {
            const response = await window.electron.ipcRenderer.UpdateClient(client);
            setIsSuccess(true);
            setResult(response);
        } catch (err) {

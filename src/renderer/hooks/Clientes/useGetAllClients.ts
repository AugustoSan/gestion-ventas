...
export const useGetAllClients = (isValid: boolean): IDataResponse => {
  const [result, setResult] = useState<Array<IClient> | null>(null);
  ...
  useEffect(() => {
    if (isValid && calls === 0) {
      ...
      const callContractFunction = async (): Promise<void> => {
        setIsLoading(true);
        try {
            const response = await window.electron.ipcRenderer.GetAllClients();
            setIsSuccess(true);
            setResult(response);
        } catch (err) {

...
export const useGetVentaById = ({isValid, id}: IDataRequestFindById): IDataResponseAllVentas => {
const [result, setResult] = useState<IVenta | null>(null);
...
useEffect(() => {
  if (isValid && calls === 0) {
    ...
    const callContractFunction = async (): Promise<void> => {
      setIsLoading(true);
      try {
          const response = await window.electron.ipcRenderer.GetVentaById(id);
          setIsSuccess(true);
          setResult(response);
      } catch (err) {
